...
async function init(){
  ...
  await loadFilters();     // carga filtros
...
async function loadFilters(){
  try{
    ...
    // ⬇️ Owners ahora: value=id, label=name
    ...
    setOwnerBtnLabel(); // usa nombres, guarda IDs
...
function normalizeFilters(json){
  ...
  const owners    = (json.owners||[]).map(o=>({ id: o.id ?? o.value ?? o.name, name: o.name ?? String(o.value||'') || 'Unknown' }));
  ...
  return { clients, owners, platforms, statuses, projects };
}
...
function renderMenu(container, list, key, labelFn, valueFn, opts={multi:true, searchable:true, initials:false}){
  ...
  if(opts.searchable){
    const sb = document.createElement('div'); sb.className = 'search';
    ...
    sb.appendChild(input); sb.appendChild(clear); container.appendChild(sb);
      ...
      .forEach(it=>{
        ...
        div.addEventListener('click', (e)=>{
          ...
          if(key==='statuses'){ // single-select
            ...
            refresh(true); // mantenemos abierto en multi-select
          }
        });
...
function filterProjectsForClients(){
  ...
  let show = all;
  if(clients.length){ show = all.filter(p => p.clientIds && p.clientIds.some(id => clients.includes(id))); }
...
function updateButtonsText(){
  ...
  const activeCount = state.selected.clients.length + state.selected.projects.length + state.selected.platforms.length + state.selected.owners.length + state.selected.statuses.length;
...
async function fetchMore(replace){
  try{
    ...
    state.selected.owners.forEach(v=>params.append('owner', v));     // ⬅️ ahora IDs
...
function placeholderList(n){ if(!n) return ''; return Array.from({length:n}, ()=>`<div class="placeholder">No content</div>`).join(''); }
function skeleton(n){ return placeholderList(n); }
...
function openModal(id){
  ...
  els.vCopy.textContent = copy; els.vCopy.hidden = !copy;
  renderModal();
...
function toast(msg, ms=2500){ const div = document.createElement('div'); div.className='toast'; div.textContent=msg; els.toasts.appendChild(div); setTimeout(()=>div.remove(), ms); }

function fmtDate(iso){ const d = new Date(iso); if(isNaN(d)) return ''; const m = MONTHS[d.getMonth()]; const dd = String(d.getDate()).padStart(2,'0'); return `${m} ${dd}`; }
function escapeHtml(s){ return (s||"").replace(/[&<>"]/g, c=>({ '&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;' }[c])); }
function initials(name){ const n=(name||'').trim(); if(!n) return '??'; const parts=n.split(/\s+/); const a=(parts[0]||'')[0]||''; const b=(parts[1]||'')[0]||''; return (a+b || a).toUpperCase(); }
const OWNER_COLORS=['#10B981','#8B5CF6','#EC4899','#F59E0B','#3B82F6','#EF4444','#FCD34D','#14B8A6','#A855F7','#22C55E'];
function ownerColor(name){ const s=(name||'').trim(); if(!s) return '#6B7280'; let h=0; for(let i=0;i<s.length;i++) h=(h*31 + s.charCodeAt(i))|0; const idx=Math.abs(h)%OWNER_COLORS.length; return OWNER_COLORS[idx]; }

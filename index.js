...
async function init(){
  // leer parámetros de la URL (para el caso de portales de cliente)
  const url = new URL(window.location.href);
  const pClient  = url.searchParams.getAll('client');
  const pProject = url.searchParams.getAll('project');

  if (pClient.length)  state.selected.clients  = pClient;
  if (pProject.length) state.selected.projects = pProject;
  ...
  await loadFilters();
  // al cargar filtros, actualizamos etiquetas si venían de la URL
  updateButtonsText();
  await refresh(true);
}
...
async function loadFilters(){
  try{
    ...
    if (!state.selected.clients.length)   setBtnText(els.fClient, "All Clients");
    if (!state.selected.projects.length)  setBtnText(els.fProject, "All Projects");
    if (!state.selected.platforms.length) setBtnText(els.fPlatform, "All Platforms");
    if (!state.selected.owners.length)    setBtnText(els.fOwner, "All Owners");
    if (!state.selected.statuses.length)  setBtnText(els.fStatus, "All Status");
...
function mapPostShape(p){
  ...
  return {
    ...
    copy: p.copy || '',
    media: assets.map(a => ({
      type: a.type === 'video' ? 'video' : a.type === 'external' ? 'external' : 'image',
      url: a.url || ''
    }))
  };
}
...
function renderCard(p){
  ...
  const badges = `
    <div class="card__badges">
      ...
      ${isExternal? `<span class="badge-ico" title="Embed">↗</span>` : ``}
  ...
  if (first){
    if (isVideo){
      mediaEl = `<video class="card__media" preload="metadata" muted playsinline src="${escapeHtml(first.url)}"></video>`;
    } else if (isExternal){
      mediaEl = `<div class="card__media card__media--embed">
        <span>External</span>
      </div>`;
    } else {
    ...
    mediaEl = `<div class="placeholder">No content</div>`;
  }
...
function renderModal(){
  ...
  let inner = '';
  if (a.type==='video'){
    inner = `<video preload="metadata" controls playsinline src="${escapeHtml(a.url)}" style="max-width:100%;max-height:60vh;object-fit:contain"></video>`;
  } else if (a.type==='external'){
    inner = `<iframe src="${escapeHtml(a.url)}" style="width:100%;height:60vh;border:0;" allow="autoplay; fullscreen"></iframe>`;
  } else {
    inner = `<img alt="" src="${escapeHtml(a.url)}" style="max-width:100%;max-height:60vh;object-fit:contain" />`;
  }
  els.vStage.innerHTML = inner;
